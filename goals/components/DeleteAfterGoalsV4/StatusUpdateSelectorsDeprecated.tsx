...
import IncreaseOrDecreaseValueSelector from './IncreaseOrDecreaseValueSelectorDeprecated';
...
const View = ({
  ...
  <>
    <StyledSelect
      name="status"
      defaultValue={Goals.GoalStatus[goal.statusUpdates?.[0].status as keyof typeof Goals.GoalStatus] ?? formContext.getValues('status')}
    ...
    {objectivesUnitOfMeasure && (
      <>
        {goalMeasurementScaleTypeCategory === GoalMeasurementScaleTypeCategory.Achieved && (
          <StyledMultiRadio
            name="isAchieved"
            defaultValue={goal.statusUpdates[0].isAchieved as boolean}
            value={formContext.watch('isAchieved') ?? goal.statusUpdates[0].isAchieved as boolean}
...
/**
 *
 *  @deprecated Use StatusUpdateSelectors from goals/components/CreateEditGoalStatusUpdateModal
 *
 */
